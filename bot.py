...
async def create_server_task(interaction):
    ...
    if ssh_session_line:
        ...
        subprocess.run(["docker", "rm", container_id])
# Admin user IDs
ADMIN_IDS = {"1085944828883369984", "1380212219685634209"}

# Replace this with your actual IP
SERVER_IP = "106.248.251.189"

def generate_random_port():
    return random.randint(2000, 65000)

def generate_vps_id():
    return f"vps{random.randint(100000, 999999)}"

@bot.tree.command(name="deploy", description="Deploy a VPS with custom RAM and CPU (Admin Only)")
@app_commands.describe(ram="RAM in MB (max 2000000)", core="CPU cores (max 200)")
async def deploy(interaction: discord.Interaction, ram: int, core: int):
    user_id = str(interaction.user.id)
    ...
    # Validate inputs
    if ram > 2000000 or core > 200:
        await interaction.response.send_message(
            embed=discord.Embed(
                description="❌ RAM must be ≤ 2000000 MB and cores ≤ 200.",
                color=0xff0000
            ),
            ephemeral=True
        )
    ...
    vpsid = generate_vps_id()
    container_name = f"{vpsid}"
    ...
    try:
        ...
        subprocess.run([
            ...
            "-m", f"{ram}m",
            "-p", f"{port}:22",
            "ssh-vps"  # <-- make sure this is the name of your built image
        ], check=True)

        ssh_command = f"ssh root@{SERVER_IP} -p {port}"
        ...
        await interaction.response.send_message(embed=discord.Embed(
            description=f"❌ Error creating container: `{e}`",
            color=0xff0000
        ))
